...
float OneMax::evaluate(const vector<bool> & solution) {
    ofstream myfile;
    myfile.open ("individual.txt");
    for (const bool & bit : solution) {
        myfile << bit;
    }
    myfile.close();
    int sysRet = system("./ml_launcher.sh");
    if (sysRet == -1) {
      // system failed
    }

    // read the results given by the ml_qip fitness function
    float fit = 0;
    float x = 0;
    ifstream inFile;
    inFile.open("fitness.txt");
    while (inFile >> x) {
        fit = x;
    }
    // return the fitness value after evaluating the individual
    return (-1 * fit);
}
